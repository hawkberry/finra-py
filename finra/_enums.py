...
class Groups(Enum):
    ...
    #: only accessible by the firm itself.
    FIRM = "firm"
    ...
    #: String: TRACE Report Card API group. These datasets provides access to
...
class EquityGroup(Enum):
    """
    Equity API group. These datasets provide access to Over-the-Counter (OTC)
    trade and equity data.
    """
    
    #: String: Aggregated ATS trade data in NMS stocks that meets certain share
    #: based and dollar based thresholds
    ATS_BLOCK_SUMMARY = "blocksSummary"
    
    #: String: Aggregated OTC (Non-ATS) trade data in NMS stocks that meets
    #: certain share based and dollar based thresholds
    OTC_BLOCK_SUMMARY = "otcBlocksSummary"
    
    #: String: OTC short interest positions submitted across all exchanges
    CONSOLIDATED_SHORT_INTEREST = "consolidatedShortInterest"
    
    #: String: Daily short sale aggregated volume by security for all short
    #: sale trades executed and reported to a FINRA trade reporting facility
    DAILY_SHORT_SALE_VOLUME = "regShoDaily"
    
    #: String: OTC Regulation SHO and Rule 4320 Threshold Securities
    THRESHOLD_LIST = "thresholdList"
    
    #: String: Weekly aggregate trade data for OTC (ATS and non-ATS)
    #: trading data for each ATS/firm with trade reporting obligations under
    #: FINRA rules
    WEEKLY_SUMMARY = "weeklySummary"
    
    #: String: Historic Weekly aggregate trade data for OTC (ATS and non-ATS)
    #: trading data for each ATS/firm with trade reporting obligations under
    #: FINRA rules
    WEEKLY_SUMMARY_HISTORIC = "weeklySummaryHistoric"
    
    #: String: Monthly aggregate trade data for OTC (non-ATS) trading data for
    #: each firm with trade reporting obligations under FINRA rules
    MONTHLY_SUMMARY = "monthlySummary"
    
    #: String: OTC Daily List
    OTC_DAILY_LIST = "otcDailyList"


class FixedIncomeGroup(Enum):
    """
    Fixed Income API group. These datasets provide access to Over-the-Counter
    (OTC) secondary market transaction data for fixed income securities as
    reported to TRACE.
    """
    
    #: String: Daily pricing statistics for Agency Pass-Thru MBS traded on TBA,
    #: STIP, and Dollar Roll basis
    AGENCY_TBA_PRICING = "agencyTBAPricing"
    
    #: String: Daily pricing statistics for Agency CMO by deal vintage
    AGENCY_CMO_PRICING = "agencyCMOPricing"
    
    #: String: TRACE market breadth calculations for Agency debt securities
    AGENCY_DEBT_MARKET_BREADTH = "agencyMarketBreadth"
    
    #: String: TRACE market sentiment calculations for Agency debt securities
    AGENCY_DEBT_MARKET_SENTIMENT = "agencyMarketSentiment"
    
    #: String: Daily trading activity statistics for Agency MBS
    AGENCY_MBS_TRADING_ACTIVITY = "agencyMBSTradingActivity"
    
    #: String: Daily pricing statistics for Agency Pass-Thru ARM and hybrid MBS
    AGENCY_MBS_ARM_HYBRID_PRICING = "agencyMBSArmHybridPricing"
    
    #: String: Daily pricing statistics for specified Agency Pass-Thru
    #: fixed-rate MBS
    AGENCY_MBS_PRICING = "agencyMBSPricing"
    
    #: String: Daily pricing statistics for CBOs, CDOs, and CLOs by credit tier
    #: and vintage
    COLLATERALIZED_OBLIGATIONS_PRICING = "collateralizedObligationPricing"
    
    #: String: TRACE market breadth calculations for Rule 144A corporate debt
    #: securities
    CORPORATE_144A_DEBT_MARKET_BREADTH = "corporate144AMarketBreadth"
    
    #: String: TRACE market sentiment calculations for Rule 144A corporate debt
    #: securities
    CORPORATE_144A_DEBT_MARKET_SENTIMENT = "corporate144AMarketSentiment"
    
    #: String: TRACE capped volume calculations for corporate and agency debt
    CORPORATE_AND_AGENCY_CAPPED_VOLUME = "corporatesAndAgenciesCappedVolume"
    
    #: String: TRACE market breadth calculations for corporate debt securities
    CORPORATE_DEBT_MARKET_BREADTH = "corporateMarketBreadth"
    
    #: String: TRACE market sentiment calculations for corporate debt
    #: securities
    CORPORATE_DEBT_MARKET_SENTIMENT = "corporateMarketSentiment"
    
    #: String: Daily pricing statistics for CMBS by deal vintage
    DAILY_CMBS_PRICING = "dailyCMBSPricing"
    
    #: String: Daily pricing statistics for Non-Agency CMO and ABS by product
    NON_AGENCY_CMO_ABS_PRICING = "nonAgencyCMOABSPricing"
    
    #: String: Daily pricing statistics for Non-Agency CMO P&I securities by
    #: deal vintage
    NON_AGENCY_CMO_PRICING = "nonAgencyCMOVintagePricing"
    
    #: String: TRACE capped volume calculations for securitized products
    SECURITIZED_PRODUCTS_CAPPED_VOLUME = "securitizedProductsCappedVolume"
    
    #: String: Cumulative log of corrections to previously published
    #: securitized products data
    SECURITIZED_PRODUCTS_ERRATA = "securitizedProductErrata"
    
    #: String: Daily trading activity statistics for securitized products other
    #: than Agency Pass-Thru MBS, including Non-Agency MBS and CMBS
    SECURITIZED_PRODUCTS_TRADING_ACTIVITY = "securitizedProductTradingActivity"
    
    #: String: Daily trading volume in US Treasury Securities reported to
    #: TRACE
    TREASURY_DAILY_AGGREGATES = "treasuryDailyAggregates"
    
    #: String: Monthly trading volume in US Treasury Securities reported to
    #: TRACE
    TREASURY_MONTHLY_AGGREGATES = "treasuryMonthlyAggregates"
    
    #: String: Weekly pricing statistics for Agency CMBS by deal vintage,
    #: aggregated across a full Monday-to-Friday trading week
    WEEKLY_CMBS_PRICING = "weeklyCMBSPricing"



class FINRAGroup(Enum):
    """
    FINRA API group. These datasets include data and content typically found on
    FINRA.org including the FINRA Rulebook.
    """
    
    #: String: FINRA's current and historical rules and regulations
    FINRA_RULE_BOOK = "finraRulebook"
    
    #: String: Industry snapshot of firm registration types. Breakdown of
    #: securities industry registered firms by type of registration.
    FIRM_REGISTRATION_TYPES = "industrySnapshotFirmsByRegistrationType"


class FirmGroup(Enum):
    """
    Firm API group. These datasets provide access to information that is
    specific to individual FINRA member firms, some of which are only
    accessible by the firm itself.
    """
    
    #: String: Form 4530 customer complaint filings against the firm
    FIRM_CUSTOMER_COMPLAINTS = "4530filings"
    
    #: String: Firm disclosure information
    FIRM_DISCLOSURES = "firmDisclosures"
    
    #: String: Basic information about a firm
    FIRM_PROFILE = "firmProfile"
    
    #: String: Complete listing of registration notices for a firm
    FIRM_REGISTRATION_STATUS_HISTORY = "firmRegistrationStatusHistory"
    
    #: String: Firm registration information
    FIRM_REGISTRATIONS = "firmRegistrations"


class RegistrationGroup(Enum):
    """
    Registration API group. These datasets provide member firms access to their
    registration records as stored in the Central Registration Depository
    (CRD).
    """
    
    #: String: CRD accounting transactions of the requesting firm for a given
    #: date range
    ACCOUNTING = "accounting"
    
    #: String: Altered identifiers of registered individuals (only available in
    #: the QA Test Environment)
    ALTERED_SSN_AND_DOB = "alteredSSNandDOB"
    
    #: String: Identify the requesting firm's branches where information has
    #: changed within a given datetime range
    BRANCH_DELTA = "branchDelta"
    
    #: String: List active branches for the requesting firm
    BRANCH_LIST = "branchList"
    
    #: String: All broker-dealer firms that are registered with and regulated
    #: by FINRA
    BROKER_DEALER_FIRM_LIST = "brokerDealerFirmList"
    
    #: String: Comprehensive real-time data that FINRA has for a given Branch
    #: CRD Number
    COMPOSITE_BRANCH = "compositeBranch"
    
    #: String: Comprehensive real-time data that FINRA has for a given
    #: Individual CRD Number
    COMPOSITE_INDIVIDUAL = "compositeIndividual"
    
    #: String: JSON file download of all the individuals currently employed by
    #: the requesting firm, and individuals terminated within the last 30 
    #: calendar days
    COMPOSITE_INDIVIDUAL_SEED = "compositeIndividualSeed"
    
    #: String: FINPRO tasks/alerts for the requesting firm's registered
    #: representatives
    FINPRO_TASKS = "finproTasks"
    
    #: String: JSON file download of annual renewal fees charged per registered
    #: individual
    FIRM_RENEWAL = "firmRenewal"
    
    #: String: Identify individual representatives whose information has
    #: changed within a given date and time range
    INDIVIDUAL_DELTA = "individualDelta"
    
    #: String: Comprehensive real-time fingerprint data that FINRA has for a
    #: given individual
    INDIVIDUAL_FINGERPRINT = "fingerprint"
    
    #: String: Registration records of industry individuals being considered
    #: for employment
    INDIVIDUAL_PRE_REGISTRATION_SEARCH = "preRegistrationIndividual"
    
    #: String: Registration records of individuals in the industry being
    #: considered for employment, along with complete disclosure information.
    #: (Depricated. Replaced by U4 Form Prefill)
    INDIVIDUAL_PRE_REGISTRATION_SEARCH_V2 = "preRegistrationIndividualv2"
    
    #: String: Validate the registration status of a registered individual with
    #: FINRA and the states independent of what firm they are affiliated with
    INDIVIDUAL_REGISTRATION_VALIDATION = "registrationValidationIndividual"
    
    #: String: Validate the registration status including disclosure details of
    #: a registered individual with FINRA and the states independent of what
    #: firm they are affiliated with
    INDIVIDUAL_REGISTRATION_VALIDATION_DETAILS = \
        "IndividualRegistrationValidationDetails"
    
    #: String: CRD numbers for registered individuals
    REGISTERED_INDIVIDUAL_SEARCH = "RegisteredIndividualSearch"
    
    #: String: Data to pre-populate Form U4 initial fields when submitting
    #: registration applications through the Submission API.
    #: (Replaces Pre-Registration Individual v2)
    U4_FORM_PREFILL = "U4FormPrefill"


class ReportCardGroup(Enum):
    """
    TRACE Report Card API group. These datasets provides access to FINRA TRACE
    Report Cards available via FINRA Report Center. Report Cards help firms
    detect potential compliance issues early and cover a variety of topics and
    rule sets. Learn more about `TRACE Report Cards
    <https://www.finra.org/compliance-tools/report-center>`__.
    """
    
    #: String: TRACE quality of markets report cards for Agency Debt Summary
    TRACE_AGENCY_DEBT_SUMMARY = "traceAgencySummary"
    
    #: String: TRACE quality of markets report cards for Treasuries Summary
    TRACE_TREASURIES_SUMMARY = "traceTreasuriesSummary"
    
    #: String: TRACE quality of markets report cards for Corporate Bonds
    #: Summary
    TRACE_CORPORATE_BONDS_SUMMARY = "traceCorporateBondSummary"
    
    #: String: TRACE quality of markets report cards for Securitized Products
    #: Summary
    TRACE_SECURITIZED_PRODUCTS_SUMMARY = "traceSecuritizedProductSummary"
